...
3. Agree what done means before building: acceptance criteria, non-goals, the signatures others will call, the edge cases, the check. Several tasks → each names the criterion it satisfies. Ambiguity that changes the work → ask once, now. A revised plan keeps every earlier correction.
...
6. Build until they pass, without touching them. No code for cases that cannot happen; nothing the outcome did not name unless you can say what breaks without it.
